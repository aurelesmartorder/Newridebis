  ...
  try {
    ...
    const sessionConfig = {
      session: {
        ...
        audio: {
          input: {
            // (CORRECTIF) Sans ces réglages explicites, l'API utilise ses
            // valeurs par défaut — trop sensibles pour un téléphone tenu à
            // distance en extérieur (vent, bruits de carrière, cheval).
            // Résultat observé : le coach réagissait à du bruit ambiant
            // comme si le cavalier avait parlé, ou coupait une phrase pour
            // en recommencer une autre.
            turn_detection: {
              type: 'server_vad',
              threshold: 0.6,          // plus haut que le défaut (~0.5) : moins sensible au bruit de fond
              prefix_padding_ms: 300,
              silence_duration_ms: 500,
            },
            // (RETIRÉ) Un réglage transcription.language avait été ajouté ici pour
            // verrouiller la langue, mais sans pouvoir le tester contre la vraie API
            // en direct, et face à un blocage total de la conversation vocale signalé
            // juste après ce changement, il est plus sûr de revenir en arrière que de
            // deviner une nouvelle syntaxe à l'aveugle. Le verrou de langue reste actif
            // via l'instruction textuelle en tête d'ALI_IDENTITY (approche recommandée
            // par la documentation officielle d'OpenAI, sans risque de casser la
            // création de session).
            // far_field : pensé pour un micro qui n'est pas collé à la bouche
            // (téléphone en poche/support, contrairement à un casque) —
            // filtre le bruit ambiant avant même la détection de parole.
